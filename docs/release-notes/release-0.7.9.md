...
Submit bug reports and enhancement requests to me as text files (separate files for bugs and enhancements, please) on an Amiga disk. As Carl promised (the guy never fails me), he has set up a bulletin board system for us to which we can post files and from which we can download new releases at our convenience.
